...
class Usuario(db.Model):
    ...
    pantallas_asignadas = db.relationship(
        'Pantalla', backref='recepcionista', lazy='dynamic',
...
def _crear_datos_iniciales():
    """Solo se ejecuta cuando la DB está vacía por primera vez."""
    try:
        admin = Usuario(
        ...
        for i in range(1, 7):
            db.session.add(Pantalla(
                numero=i, nombre=f'Pantalla {i}',
                estado='disponible', created_by='sistema'
            ))

        db.session.commit()
        print('✅ Datos iniciales creados (admin, recepcion, 6 pantallas)')
...
def init_db(app):
    ...
    with app.app_context():
        ...
        try:
            ...
            return  # sin DB no tiene sentido continuar
        ...
        # ── 3. Datos iniciales (solo si la tabla está vacía) ───
        try:
            if Usuario.query.count() == 0:
                print('🔄 BD vacía — creando datos iniciales...')
                _crear_datos_iniciales()
            else:
                print('✅ Datos iniciales ya existen')
        except Exception as e:
            print(f'⚠️  Error al verificar datos iniciales: {e}')
